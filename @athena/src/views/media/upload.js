...
export default class UploadModal extends Component {
  state = {};
  getUploadPops = () => (!this.props.data.cloudinaryRequest ? {} : {
    showUploadList: true,
    data: {
    ...
    onChange: (info) => {
      if (info.file.status === 'uploading') {
        ...
        this.props.done({ id: file.id, token: this.props.data.cloudinaryRequest.signature }).then(() => {
  ...
  render() {
    ...
    return modal ? (
      <Modal {...modalSettings} title="Upload" onCancel={onClose} onOk={onClose} footer={[]}>
        {inner}
      </Modal>
    ) : inner;
  }
