...
export default class Analytics extends Component {
  ...
  updateDates(date) {
    ...
    this.setState({
      query: {
        ...query,
        'start-date': date && date.dateStart
          ? moment(date.dateStart).format('YYYY-MM-DD')
          : query['start-date'],
        'end-date': date && date.dateEnd
  ...
  render() {
    ...
    return (
      <div
        className="ui divided equal height grid full-h"
        style={{ margin: 0 }}
      >
        <div
          className="two wide column full-h"
          style={{ padding: 0, backgroundColor: '#F6F7F9', left: 0 }}
        >
