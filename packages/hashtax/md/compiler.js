...
function remarkReact(options) {
    ...
    function compile(node, key) {
        var tag = node.tag, type = node.type, position = node.position, depth = node.depth, value = node.value, ordered = node.ordered, url = node.url, _a = node.props, props = _a === void 0 ? {} : _a, _b = node.children, children = _b === void 0 ? [] : _b;
        var t = tag || type;
        if (!props.key)
            props.key = key;
        if (!props.value)
        ...
        return h(t, props, children.map(function (child, key) { return compile(child, key); }));
    }
