...
export default class AuthRegister extends Component {
  ...
  render() {
    const { isOpen, form, saving, pathname, onClose, extraFields, data, token } = this.props;
    const { getFieldDecorator } = form;
    const loading = token && !data.error && data.valid !== true && data.valid !== false;
    const valid = token && !data.error && data.valid !== false;

    return (
      <Base isOpen={isOpen} title="Registrieren" onOk={data.valid ? this.ok : null} onCancel={onClose} loading={loading ? 'Prüfe Token ...' : false}>
        {!valid && token && <p style={{ textAlign: 'center' }}>
          Das Token ist ungültig oder abgelaufen. Bitte <Link to={{ pathname, query: { feedback: null, confirm: undefined } }}>kontaktieren Sie den Support</Link>.
        </p>}
        {!valid && !token && <p style={{ textAlign: 'center' }}>
          Sie benötigen eine gültige Einladung um sich zu registrieren. <Link to={{ pathname, query: { feedback: null, confirm: undefined } }}>Kontaktieren Sie den Support</Link>.
        </p>}
        {valid && <Form.Item key="name" label="Name" {...layout}>
        ...
        {valid && <Form.Item key="email" label="E-Mail" {...layout}>
          {getFieldDecorator('email', {
            initialValue: data.email,
            rules: [{ required: true, message: 'Bitte geben Sie Ihre E-Mail an!' }],
          })(<Input type="email" disabled={!!data.email} placeholder="E-Mail" onKeyPress={onEnterFocus(() => this.pw1)} ref={x => this.mail = x} size="large" addonAfter={<EnvelopeO size={10} />} />)}
        </Form.Item>}
        {valid && <Form.Item key="password" label="Passwort" {...layout}>
        ...
        {valid && <Form.Item key="password2" label="Wiederholen" {...layout}>
        ...
        {valid && extraFields ? extraFields({ layout, getFieldDecorator, state: this.state, setState: this.setState }) : null}
        <Modal.Links>
