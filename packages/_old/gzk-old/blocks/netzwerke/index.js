...
export default class GZNetzwerke extends Component {
  ...
  renderItem = ({ slug, website, peak, kurz, name, farbe, id, tags, bild }, index) => (
    <a href={website} rel="noopener noreferrer" className={cn('col-md-2 text-center grayscale', !index && `offset-md-${6 - this.props.items.length}`)} key={id}>
      {/* <h5 className="mb-0 gz-simple-header"><b>{name}</b></h5>*/}
      <CloudinaryImage className="grayscale p-2" value={bild} options={{ width: 200, height: 200, mode: 'pad' }} width="100%" height="auto" />
      <b>{name}</b>
      <br />
  ...
  render() {
    ...
    return (
      <div style={{ backgroundColor: 'rgba(245, 245, 245, 0.66)' }} className="pb-1 mt-2 mb-3">
        <GenericBlock {...this.props} className="container text-center" toolbarStyle={{ marginLeft: -11, marginRight: -11 }}>
