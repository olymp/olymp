...
export default (props) => {
  const { Wrapped, collection, flatNavigation, query, pathname, modules, router } = props;
  ...
  return (
    <View
      ...
      key={query[`@${key}`]}
      id={query[`@${key}`]}
      handleListClick={collection => router.push({ pathname, query: { ...query, [`@${key}`]: collection.id }})}
      onClose={() => router.push({ pathname, query: { ...query, [`@${key}`]: undefined } })}
      pageId={pageId || id}
      render={children => (
