...
export default (title, centered, customStyle) => (subtitle, compFn, text) => storiesOf(title, module)
  ...
  .addDecorator(storyFn => centered ? <Container customStyle={customStyle || {}}>{storyFn()}</Container> : <div>{storyFn()}</div>)
  ...
  .addWithInfo(
    ...
    compFn,
  {
    // https://github.com/storybooks/storybook/blob/master/addons/info/src/components/Story.js
    styles: style => ({
      ...style,
      info: {
        ...style.info,
