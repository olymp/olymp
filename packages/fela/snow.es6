class Snow {
  constructor() {
    ...
    this.events = (function() {
      let old = !window.addEventListener && window.attachEvent,
      ...
      this.melt = function() {
        if (!storm.useMeltEffect || !s.melting) {
          ...
          s.o.style.fontSize = `${s.fontSize -
            s.fontSize * (s.meltFrame / s.meltFrameCount)}px`;
          s.o.style.lineHeight = `${storm.flakeHeight +
            2 +
            storm.flakeHeight * 0.75 * (s.meltFrame / s.meltFrameCount)}px`;
          s.meltFrame++;
        } else {
          s.recycle();
        }
