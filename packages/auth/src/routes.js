...
export default withRouter(props => {
  ...
  return (
    <div>
      ...
      <AuthConfirm {...p} isOpen={inQuery('confirm')} token={query.confirm} onOk={({ email }) => redirect({ login: email })} />
      <AuthStatus {...p} isOpen={inQuery('status-forgot')} text={texts.forgot} />
      <AuthStatus {...p} isOpen={inQuery('status-register')} text={texts.register} />
      ...
      <AuthInvitations {...p} isOpen={inQuery('@invitations')} id={query['@invitations']} />
    </div>
  );
});
