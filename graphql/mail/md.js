...
function compiler(handlers) {
  ...
  function compile(node, key, parent) {
    ...
    return handler(node, (node.children || []).map((child, key) => compile(child, key, node)), parent);
  }
}
...
module.exports = {
  toHTML: (value, options) => {
    const handlers = {
      ...
      link: (props, children, parent) => parent.type === 'paragraph' ? `
      ...
      heading: (props, children) => handlers.paragraph({}, `<h${props.depth}>${children.join('')}</h${props.depth}>`),
    ...
    return compose(remark().use(compiler, handlers).processSync(value).contents, options);
  },
  toPlain: value => {
    const handlers = {
      root: (props, children) => children.join('\n'),
