...
const resolveInt = (uri) => {
  if (uri.indexOf('./') === 0) return path.resolve(appRootDir.get(), uri);
  else if (uri.indexOf('@/') === 0) return path.resolve(base, uri.replace('@/', './'));
  return uri;
};
const resolve = (uri) => {
  const p = resolveInt(uri);
  console.log('RESOLVE', base, uri, p);
  return p;
};
  ...
  const webpackConfig = {
    ...
    resolve: {
      alias: {
        ...
        'olymp-core': path.resolve(base),
        'olymp-cms': path.resolve(appRootDir.get(), 'node_modules', 'olymp-cms'),
