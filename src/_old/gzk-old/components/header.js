...
class Menu extends Component {
  ...
  renderPlus = (props) => {
    ...
    return (
      this.renderUtil(renderPlus, { ...props, to }) ||
  ...
  renderHandle = (props) => {
    ...
    return (
      <SortHandle>
        {this.renderUtil(renderHandle, props) ||
          <span className="grippy" style={style} />}
      </SortHandle>
    );
  };
  renderItem = (props, index) => {
    ...
    const hasChildren =
      ...
      (page.headings && page.headings.length > 1);
    const classes = cn(
      { 'nav-item': !level },
      !level && hasChildren && 'dropdown-mainmenu',
      level && hasChildren && 'dropdown-submenu'
    );
    ...
    return (
      <SortElement key={page.id} index={index}>
        {this.renderUtil(renderItem, props) ||
          <li
      ...
      >
        {this.renderUtil(renderMenu, this.props) ||
          <ul
            className={cn(
              !level
                ? 'nav navbar-nav pull-right sf-menu'
                : 'dropdown-menu pl-1 pr-1'
            )}
            style={{ top: 'initial' }}
          >
  ...
  render() {
    ...
    return (
      <Menu
        ...
        page={{ children: (items || []).filter(x => x.slug !== '/') }}
      />
    );
  }
}
