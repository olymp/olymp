...
export default class BlockEinrichtungen extends Component {
  ...
  renderImage = (item, size) => (
    <div className="pl-1 pr-1 gz-height-250" key={item.id}>
      <Image onImageClick={() => this.props.router.push(item.slug)} value={item.peak} container="div" height="100%" width="100%" className="gz-image-box" cloudinary={{ width: (300 * 1.2) * size, height: 250 }}>
  ...
  render() {
    ...
    return (
      <GenericBlock {...this.props} className="gz-element" style={{ width: '100%' }}>
