...
export default class AuthReset extends Component {
  ok = () => {
    ...
    form.validateFields((err, values) => {
      ...
      auth.reset(token, values.password).then(({ email }) => {
  ...
  render() {
    ...
    return (
      <Base isOpen={isOpen} title="Zurücksetzen" onOk={valid ? this.ok : null} onCancel={onClose} loading={loading ? 'Prüfe Token ...' : false}>
        ...
        {valid && <Form.Item key="password2" label="Wiederholen" {...layout}>
          {getFieldDecorator('password2', {
            rules: [{ required: true, message: 'Bitte das wiederholte Passwort angeben!' }],
          })(<Input type="password" placeholder="Password wiederholen" onKeyPress={onEnterOk(this.ok)} ref={x => this.input = x} size="large" addonAfter={<Key size={10} />}/>)}
        </Form.Item>}
        {!valid && <p style={{ textAlign: 'center' }}>
          Das Token ist ungültig oder abgelaufen. Bitte <Link to={{ pathname, query: { forgot: null, reset: undefined } }}>beantragen Sie das Zurücksetzen des Passworts erneut</Link> oder <Link to={{ pathname, query: { feedback: null, reset: undefined } }}>kontaktieren Sie den Support</Link>.
        </p>}
        <Modal.Links>
          <Link to={{ pathname, query: { login: null, reset: undefined } }}>Zur Anmeldung</Link>
        </Modal.Links>
      </Base>
    );
  }
