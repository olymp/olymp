...
export const parseComponent = raw => {
  ...
  rest.forEach(frag => {
    ...
    if (!args[currentKey]) args[currentKey] = frag;
    else args[currentKey] += ` ${frag}`;
  });
...
export const processLines = (lines, result = []) => {
  ...
  if (equalLength) { // '#' or '##' or '##...#'
    if (line.length > 1) rest.splice(0, 0, '#');
    return { lines: rest, result };
  } else if (!split[0] && split.length === 2) { // # found exactly once
    const arg = processLines(rest);
    rest = arg.lines;
    result.push({ ...parseComponent(split[1]), kind: 'block', children: arg.result });
  } else { // no # or more than one # => assume text
    ...
    split.forEach((frag, i) => { //
      ...
      if (isEven) result.push({ ...parseComponent(frag), kind: 'inline', children: [] });
      else result.push({ text: frag });
    });
  }
