...
export default WrappedComponent => class DataConnector extends Component {
  ...
  render() {
    ...
    const props = Object.keys(this.props).reduce((store, key) => {
      ...
      if (value && typeof value === 'string' && value.indexOf('{{') !== -1) { // interpolate hashtax text
        const text = interpolate(value, all);
        if (text) store[key] = <Hashtax value={text} />;
      } else if (value && typeof value === 'string' && value.indexOf('{:') !== -1) { // interpolate react element
        let k;
        interpolate(value, (v) => k = v);
        store[key] = get(all, k);
      } else if (value && typeof value === 'string' && value.indexOf('{') !== -1) { // interpolate string
