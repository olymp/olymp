...
export default class AuthMe extends Component {
  ...
  render() {
    const { isOpen, email, form, saving, pathname, onClose, extraFields, auth } = this.props;
    const { getFieldDecorator } = form;

    return (
      <Base isOpen={isOpen} title="Profil" okText="Speichern" onOk={this.ok} onCancel={onClose}>
        ...
        <Modal.Links>
          <Link to={{ pathname, query: { totp: null, profile: undefined } }}>2-Faktor Authentifizierung</Link>
        </Modal.Links>
      </Base>
    );
  }
