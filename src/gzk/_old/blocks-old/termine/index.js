    ...
    <div className="widget">
      ...
      <div className="list-group">
        {list.map(item =>
          ...
          >
            <b>
              {moment(item.date).utcOffset('+01:00').format('DD. MMMM')}{' '}
              <small>
                {moment(item.date)
                  .utcOffset('+01:00')
                  .format('HH:mm')
    ...
    <div className="card-block">
      <div>
        ...
        <small>
          <b>
            {capitalize(
              item.art.replace('PRESSE', 'PRESSEARTIKEL').toLowerCase()
            )}
      ...
      <Link className="btn btn-secondary mt-1" to={`/news${item.slug}`}>
        Mehr erfahren
      </Link>
    </div>
  </div>;
...
export default class TermineBlock extends Component {
  render() {
    ...
    const anstehend = data.items.filter(
      x =>
        ['VORTRAG', 'VERANSTALTUNG'].includes(x.art) &&
        moment().isBefore(x.date)
    );
    const zuletzt = take(
      data.items.filter(
        x =>
    ...
    const publikationen = take(
      data.items.filter(x => ['PUBLIKATION'].includes(x.art)),
      4
    );
    ...
    return (
      <GenericBlock
        {...this.props}
        className="gz-big-element"
        style={{ width: '100%' }}
      >
        {children}
        <div
          className="row"
          style={{ ...style, margin: 'auto -15px', position: 'relative' }}
        >
