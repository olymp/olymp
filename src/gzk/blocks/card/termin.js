...
export default class GzCardTermin extends Component {
  ...
  render() {
    ...
    const list = data.items && take(data.items.filter(x => types.split(',').includes(x.art)), max);
    return (
      <GenericBlock {...this.props} className={`gz-big-element col-md-${4 * size}`} toolbarStyle={{ marginLeft: -11, marginRight: -11 }}>
        <h2>{title}</h2>
        <div className="gz-image-box gz-box-colored gz-height-250 mt-1" style={{ backgroundColor: color }}>
          <div className="list-group">
            {list && list.map(({ id, date, slug, name, art }) => (
              <Link className="list-group-item" key={id} to={`/news${slug}`} style={{ backgroundColor: 'transparent', border: 0 }}>
                <u>
                  {capitalize(art.toLowerCase().replace('presse', 'presseartikel'))}
                  {art.indexOf('P') === 0 ? ' vom ' : ' am '}
                  {moment(date).utcOffset('+01:00').format('DD. MMMM YYYY')} <small>{moment(date).utcOffset('+01:00').format('HH:mm').replace('00:00', '')}</small>
                </u>
                <br />
                {name}
              </Link>
