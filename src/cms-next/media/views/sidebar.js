...
export default class MediaSidebar extends Component {
  ...
  selectImage = (item, isActive) => {
    ...
    if (multi) {
      ...
      if (!onChange) router.replace({ pathname: location.pathname, query: { ...location.query, '@media': null } });
    } else if (onChange) {
      this.setState(
  ...
  render() {
    ...
    const leftButtons = (
      <Button.Group>
        <Button onClick={() => router.push({pathname, query: { ...query, ['@media']: undefined }})}>
          <Icon type="close" />
        </Button>
      </Button.Group>
    );
    ...
    return (
      <Splitview deviceWidth={deviceWidth}>
        ...
        {(id || (selected && selected.length)) && (
          <Detail images={selected.map(id => [].find(x => x.id === id))} />
        )}
      </Splitview>
    );
  }
