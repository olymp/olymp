...
import { Modal, List as UIList, SplitView, Panel, onEnterFocus, onEnterOk, layout, onError, onSuccess } from 'olymp/ui';
...
// Views
export const Detail = withState('text')(({ text, setText, viewType, pathname, item, form, onChange, ok, ...props }) => (
  <div>
    <UIList.Title buttons={
      <Button size="small" onClick={ok} shape="circle" icon="save" />
    }>{item.name || 'Neues Template'}</UIList.Title>
...
export const List = (props) => (
  <CollectionSplitView {...props} key={props.id} DetailView={DataDetail} typeName="Template" nameField="name" />
);
