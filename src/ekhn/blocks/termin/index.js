...
import { useGenericBlock, GenericBlock, SlateMate, Image } from 'olymp/cms';
...
export default class TerminBlock extends Component {
  renderOne = (termin) => {
    const { start, ende, name, kommentar, farbe, ganztaegig, id, bild, pfarrer, organist, additional, ort } = termin;
    ...
    return (
      <div className="termin row" style={{ backgroundColor }} key={id}>
      ...
      <GenericBlock {...rest} style={{ width: '100%' }}>
        <div className="items">
          <div className="item">
            ...
            {termine.slice((page - 1) * steps, page * steps).map(x => this.renderOne(x))}
          </div>
